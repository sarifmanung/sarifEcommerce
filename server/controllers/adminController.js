...
exports.auth = (req, res) => {
  ...
  if (user_name && password) {
    ...
    connection.query(
      "SELECT * FROM staff WHERE user_name = ?",
      [user_name],
      async (err, rows) => {
...
exports.show_admin_page = async (req, res) => {
  ...
  if (currentPage > 0 && currentPage <= lastPage) {
    // User the connection
    connection.query(
      `SELECT * FROM staff WHERE status = "active" LIMIT ${perPage} OFFSET ${offset}`,
      (err, rows) => {
        // When done with the connection, release it
        if (!err) {
...
exports.create = (req, res) => {
  ...
  function checkPhone() {
    connection.query(
      "select * from staff where phone_number = ?",
      [phone_number],
      (err, rows) => {
        if (!err) {
          if (rows.length) {
  ...
  async function saveNewStaff() {
    ...
    connection.query(
      "INSERT INTO staff SET first_name = ?, last_name = ?,user_name = ?, password = ?, IC = ?, DOB = ?,phone_number = ?, email = ?, role = ?",
      [
        first_name,
        last_name,
        user_name,
...
exports.update = (req, res) => {
  ...
  function checkUsername() {
    ...
    connection.query(
      "select user_name from staff where id = ?",
      [req.params.id],
      (err, rows) => {
        if (!err) {
          ...
          // if user_name got changed, go check got duplicate with other user user_name or not!!!!
            ...
            connection.query(
              "select user_name from staff where user_name = ?",
              [user_name],
              (err, rows) => {
                if (!err) {
                  console.log(rows.length);
      ...
      (err, rows) => {
        if (!err) {
          ...
          // if phone_number got changed, go check got duplicate with other user phone_number or not!!!!
            ...
            connection.query(
              "select phone_number from staff where phone_number = ?",
              [phone_number],
              (err, rows) => {
                if (!err) {
                  console.log(rows.length);
  ...
  async function updateStaff() {
    let sql =
      "UPDATE staff SET first_name = ?, last_name = ?, user_name = ?, IC = ?, DOB = ?, phone_number = ?, email = ?, role = ?";
    let value = [
      first_name,
      last_name,
      user_name,
...
exports.hideAdmin_Page = (req, res) => {
  ...
  // User the connection
  connection.query(
    'SELECT * FROM staff WHERE status = "removed"',
    (err, rows) => {
      // When done with the connection, release it
      if (!err) {
        let removedUser = req.query.removed;
...
exports.white_list = async (req, res) => {
  ...
  if (currentPage > 0 && currentPage <= lastPage) {
    // User the connection
    connection.query(
      `SELECT * FROM white_list LIMIT ${perPage} OFFSET ${offset}`,
      (err, rows) => {
        // When done with the connection, release it
        if (!err) {
          let hidePagination = false;
...
exports.search_staff = (req, res) => {
  ...
  // User the connection
  connection.query(
    "SELECT * FROM staff WHERE first_name LIKE ? OR last_name LIKE ? OR user_name LIKE ? OR role LIKE ? OR DOB LIKE ? OR IC LIKE ? OR phone_number LIKE ? OR email LIKE ?",
    [
      "%" + x + "%",
      "%" + x + "%",
      "%" + x + "%",
...
exports.search_white_list = (req, res) => {
  ...
  // User the connection
  connection.query(
    "SELECT * FROM white_list WHERE name LIKE ? OR phone_number LIKE ? OR app_id_number LIKE ?",
    ["%" + x + "%", "%" + x + "%", "%" + x + "%"],
    (err, rows) => {
      if (!err) {
        let hidePagination = true;
