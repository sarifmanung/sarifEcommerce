...
exports.create = (req, res) => {
  ...
  function checkAppID() {
    connection.query(
      "select * from customer where app_id_number = ?",
      [app_id_number],
      (err, rows) => {
        if (!err) {
          if (rows.length) {
  ...
  function checkBarcode_number() {
    connection.query(
      "select * from customer where barcode_number = ?",
      [barcode_number],
      (err, rows) => {
        if (!err) {
          if (rows.length) {
...
exports.update = (req, res) => {
  ...
  function checkAppID() {
    ...
    connection.query(
      "select app_id_number from customer where id = ?",
      [req.params.id],
      (err, rows) => {
        if (!err) {
          ...
          // if app_id_number got changed, go check got duplicate with other user app_id_number or not!!!!
            ...
            connection.query(
              "select app_id_number from customer where app_id_number = ?",
              [app_id_number],
              (err, rows) => {
                if (!err) {
  ...
  function checkIC() {
    ...
    connection.query(
      "select IC from customer where id = ?",
      [req.params.id],
      (err, rows) => {
        if (!err) {
          ...
          // if IC got changed, go check got duplicate with other user IC or not!!!!
            ...
            connection.query(
              "select IC from customer where IC = ?",
              [IC],
              (err, rows) => {
                if (!err) {
                  console.log(rows.length);
  ...
  function checkPhone() {
    ...
    connection.query(
      "select phone_number from customer where id = ?",
      [req.params.id],
      (err, rows) => {
        if (!err) {
  ...
  function checkBarcode() {
    ...
    connection.query(
      "select barcode_number from customer where id = ?",
      [req.params.id],
      (err, rows) => {
        if (!err) {
          ...
          // if barcode_number got changed, go check got duplicate with other user barcode_number or not!!!!
            ...
            connection.query(
              "select barcode_number from customer where barcode_number = ?",
              [barcode_number],
              (err, rows) => {
                if (!err) {
                  console.log(rows.length);
    ...
    (err, rows) => {
      ...
      if (rows.length != 0) {
        ...
        connection.query(
          ...
          ["%d/%m/%Y", "%d/%m/%Y", "%d/%m/%Y", "%d/%m/%Y", "%d/%m/%Y", id],
          (err, rows) => {
            if (!err) {
...
exports.test_date_format = (req, res) => {
  connection.query(
    "SELECT DATE_FORMAT( DOB , ? ) as format_date FROM customer WHERE id = ?",
    ["%d/%m/%Y", 1],
    (err, rows) => {
      if (!err) {
