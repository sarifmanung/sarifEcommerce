        ...
        try {
          ...
          for (let i = 0; i < productQuantities.length; i++) {
            ...
            if (quantity) {
              ...
              connection.query(
                "UPDATE calendar_products SET stock_quantity = stock_quantity - ? WHERE product_id = ?",
...
async function getOrderSummary(orderId) {
  try {
    const orderDetails = await new Promise((resolve, reject) => {
      connection.query(
        "SELECT oi.*, p.product_name, p.price FROM order_items oi JOIN calendar_products p ON oi.product_id = p.product_id WHERE oi.order_id = ?",
        [orderId],
        (err, results) => {
          if (err) {
            reject(err);
...
exports.productPage = async (req, res) => {
  ...
  if (currentPage > 0 && currentPage <= lastPage) {
    // User the connection
    connection.query(`SELECT * FROM calendar_products`, (err, rows) => {
      // When done with the connection, release it
      if (!err) {
...
exports.editProduct = async (req, res) => {
  ...
  // User the connection
  connection.query(
    "SELECT * FROM calendar_products WHERE product_id = ?",
    [req.params.id],
    (err, rows) => {
      if (!err) {
